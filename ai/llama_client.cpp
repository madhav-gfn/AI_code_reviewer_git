...
std::string LlamaClient::generate_commit_message(const std::string& prompt) const {
    ...
    if (llama_tokenize(vocab, formatted.c_str(), static_cast<int32_t>(formatted.size()),
                        prompt_tokens.data(), static_cast<int32_t>(prompt_tokens.size()), true,
                        true) < 0) {
        throw std::runtime_error("Failed to tokenize commit message prompt");
    }
    if (static_cast<int>(prompt_tokens.size()) >= n_ctx_) {
        throw std::runtime_error(
            "Diff is too large for commit message generation (n_ctx=" +
    ...
    std::string output;
    const int max_response_tokens = 128;  // Commit messages are short.
    int n_decoded = 0;
    int n_pos = 0;
    ...
    while (n_pos + batch.n_tokens < n_ctx_ && n_decoded < max_response_tokens) {
        if (llama_decode(ctx, batch)) {
            llama_sampler_free(sampler);
            throw std::runtime_error("llama_decode failed during commit message generation");
        }
        n_pos += batch.n_tokens;
    ...
    // Trim whitespace — the model may emit leading/trailing newlines.
    auto start = output.find_first_not_of(" \t\n\r");
    auto end = output.find_last_not_of(" \t\n\r");
    if (start == std::string::npos) return {};
    // Take only the first line (in case the model outputs extra).
